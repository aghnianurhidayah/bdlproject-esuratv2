...
$get = mysqli_query($conn, "SELECT * FROM surat JOIN spn_status_perwalian ON surat.surat_id = spn_status_perwalian.spn_akte_kelahiran_id WHERE surat_id = '$surat_id'");
$surat = [];
...
if (!(isset($_SESSION['role']))) {
    ...
    if (isset($_POST['accform'])) {
        ...
        if (is_uploaded_file($tmp_ktp) && file_exists($tmp_ktp)) {
            move_uploaded_file($tmp_ktp, "../../img/kk/" . $file_kk);
        } else {
            $file_ktp = $surat['file_ktp'];
        }

        $update_form = "UPDATE spn_status_perwalian SET nik = '$nik', no_kk = '$nokk', nama = '$nama', tgl_lahir = '$tl', jenis_kelamin = '$jk', agama = '$agama', alamat = '$alamat', kewarganagaraan = '$wn', file_ktp = '$file_ktp' WHERE spn_status_perwalian_id = '$surat_id'";
...
?>
<div class="form">
    <form action="detail/pernyataan_status.php?surat_id=<?= $surat_id ?>" method="post" enctype="multipart/form-data">
        <div class="input-box">
