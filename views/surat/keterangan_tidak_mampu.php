...
if (!isset($_SESSION['role']) || $_SESSION['role'] !== 'user') {
    ...
    if ($_SESSION['role'] == "user") {
        ...
        if (isset($_POST['submitform'])) {
            $query = "SELECT sktm_id FROM sk_tidak_mampu ORDER BY sktm_id DESC LIMIT 1";
            $result = $conn->query($query);

            if ($result && $result->num_rows > 0) {
                $row = $result->fetch_assoc();
                $last_id_num = (int)substr($row['sktm_id'], 5);
                $new_id_num = $last_id_num + 1;
            } else {
                $new_id_num = 1;
            }

            $sktm_id = "KTM-" . str_pad($new_id_num, 4, '0', STR_PAD_LEFT);
...
?>
        ...
        <body>
            <form action="surat/keterangan_tidak_mampu.php" method="post" enctype="multipart/form-data">
